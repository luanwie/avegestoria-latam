...
const MOCK_DATA = {
  kpis: {
    huevosHoy: { value: "6.114", change: "+12.5%" },
    tasaPostura: { value: "88.5%", change: "+2.3%" },
    ingresoMensual: { value: "$24.6k", change: "+8.7%" },
    mortalidad: { value: "4", change: "Bajo media" },
  },
  misGalpones: [
  ...
  lotes: [
    { nombre: "Lote A - Hy-Line Brown", galpon: "Principal", raza: "Hy-Line Brown", cantidad: 2000, edad: "28 sem", postura: "83.7%", estado: "Activo" },
    { nombre: "Lote B - Hy-Line Brown", galpon: "Principal", raza: "Hy-Line Brown", cantidad: 1500, edad: "52 sem", postura: "76.2%", estado: "Activo" },
    { nombre: "Lote C - Dekalb White", galpon: "Norte", raza: "Dekalb White", cantidad: 2000, edad: "15 sem", postura: "91.5%", estado: "Activo" },
  ],
  actividades: [
  ...
  return (
    <DashboardShell badge="DEMO">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        {[
          { label: "Huevos Hoy", value: kpis.huevosHoy.value, change: kpis.huevosHoy.change, color: "emerald" },
          { label: "Tasa de Postura", value: kpis.tasaPostura.value, change: kpis.tasaPostura.change, color: "teal" },
          { label: "Ingreso Mensual", value: kpis.ingresoMensual.value, change: kpis.ingresoMensual.change, color: "amber" },
          { label: "Mortalidad", value: kpis.mortalidad.value, change: kpis.mortalidad.change, color: "rose" },
        ].map((kpi, i) => (
          <div key={i} className={`rounded-xl p-4 bg-${kpi.color}-900/30 border border-${kpi.color}-800/30`}>
        ...
        <div className="bg-emerald-900/30 border border-emerald-800/30 rounded-xl p-5">
          ...
          <div className="h-40 flex items-center justify-center bg-emerald-950/40 rounded-lg">
            <div className="flex items-end gap-2 h-32">
              {[65, 70, 55, 80, 75, 90, 85, 95, 78, 82, 88, 92].map((h, i) => (
                <div key={i} className="flex flex-col items-center gap-1">
                  <div className="w-6 sm:w-8 bg-emerald-600/60 rounded-t" style={{ height: `${h}%` }} />
                  <div className="w-6 sm:w-8 bg-amber-600/40 rounded-t" style={{ height: `${h * 0.7}%` }} />
                </div>
              ))}
            </div>
          </div>
          <LegendItem items={[{ color: "#059669", label: "Ingresos" }, { color: "#d97706", label: "Gastos" }]} />
        </div>
        ...
        <div className="bg-emerald-900/30 border border-emerald-800/30 rounded-xl p-5">
          ...
          <div className="space-y-3">
            {MOCK_DATA.misGalpones.map((g, i) => (
              <div key={i} className="flex items-center justify-between bg-emerald-950/40 rounded-lg p-3">
                <div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead className="bg-emerald-800/20">
                <tr>
                  {["Nombre", "Galpón", "Raza", "Cant.", "Edad", "Postura", "Estado"].map((h) => (
      ...
      <div className="mt-8 text-center">
        ...
        <p className="text-xs text-stone-500 mt-3">Modo demostración — Los datos son ilustrativos</p>
      </div>
    </DashboardShell>
  );
}

function LegendItem({ items }: { items: Array<{ color: string; label: string }> }) {
  return (
